...
if((empty($mode)) or ($mode == 'main')){
	...
	if ( isset( $_GET['apage'] ) )
		...
		?>
		<script src="seetheface-btn.js" type="text/javascript"></script>
		...
		<!-- Manage Video-->
				...
				<?php
					if($tables) {
						...
						foreach($tables as $table) {
							...
							echo "<td><a href=\"javascript:ev_insertVideoCode($table->id);close();\" class=\"edit\">".__('Select')."</a></td>\n";
							echo '</tr>';
							$i++;
						}
					} else {
			...
			<h3><a href="http://www.seetheface.com/api/wordpress/?<?php echo 'api_key='.API_KEY.'&api_return='.RETURN_URL.'2.php'."\">";
